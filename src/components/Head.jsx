...
import { toggleMenu } from "../utils/navigationSlice";
import { useDispatch, useSelector } from "react-redux";
import { YOUTUBE_SEARCH_API } from "../utils/constants";
import { Link, Router, Routes, json } from "react-router-dom";
...
const Head = () => {
  ...
  return (
    <div className="grid grid-flow-col px-4 items-center shadow-lg ">
      ...
      <div className="col-span-8 flex flex-col">
        <div className="flex">
          <input
            ...
            placeholder="Search"
            className="border border-gray rounded-l-2xl p-2 border-high w-1/2 h-10"
          />
          <button className="border border-gray h-10 px-4 border-l-0 rounded-r-2xl">
            <IoIosSearch className=" h-5 w-5" />
          </button>
        </div>
        {showSuggestions && suggestions.length > 0 && (
          <div className="absolute bg-white px-3 py-2 mt-11 ml-1 rounded-lg shadow-md w-1/3 h-auto">
            <ul className="">
      ...
      <div className="flex justify-end h-6 w-6 col-span-1">
        <FaUser />
      </div>
    </div>
  );
